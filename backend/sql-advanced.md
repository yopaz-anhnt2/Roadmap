...
5. [Locking](#locking)
6. [Isolation Level](#isolation-level)
7. [Deadlock](#deadlock)
8. [Pagination](#pagination)
9. [Full Text Search](#full-text-search)
10. [Pre-aggregation](#pre-aggregation)
...
4. Tránh LIKE với wildcard ở đầu (`%abc`), nên dùng Full Text Search thay thế.
```sql
-- Dùng được index: biết chuỗi bắt đầu bằng gì nên nhảy thẳng tới vùng 'abc...'
SELECT * FROM users WHERE name LIKE 'abc%';
-- KHÔNG dùng được index: không biết ký tự đầu nên phải quét từng dòng cả bảng
SELECT * FROM users WHERE name LIKE '%abc%';
```
Vì index (B-tree) sắp xếp dữ liệu theo thứ tự từ trái sang phải, giống danh bạ sắp theo chữ cái đầu. Biết tiền tố (`abc%`) thì nhảy thẳng tới đúng vùng; còn `%abc` thì ký tự đầu là bất kỳ nên index vô dụng, phải quét cả bảng (full scan). Muốn tìm chuỗi con nằm giữa văn bản thì dùng Full Text Search (xem mục Full Text Search).
...
6. Phân trang bằng keyset thay vì OFFSET lớn.
```sql
-- OFFSET lớn: chậm vì vẫn phải đọc rồi bỏ đi 100000 dòng đầu mới lấy được 20 dòng
SELECT * FROM posts ORDER BY id DESC LIMIT 20 OFFSET 100000;
-- Keyset: dùng id của dòng cuối trang trước làm mốc, nhảy thẳng nhờ index nên nhanh
SELECT * FROM posts WHERE id < 981 ORDER BY id DESC LIMIT 20;
```
`OFFSET 100000` bắt database duyệt qua 100000 dòng đầu rồi vứt đi, chỉ giữ 20 dòng cuối, nên càng lật sâu càng chậm. Keyset lấy mốc là giá trị của dòng cuối trang trước (`WHERE id < 981`) để nhảy thẳng tới vị trí cần nhờ index, nhanh và ổn định ở mọi độ sâu. Đổi lại keyset chỉ đi tiến/lùi tuần tự, không nhảy tới trang số bất kỳ (xem mục Pagination).

7. Dùng EXPLAIN để xem database thực sự chạy query thế nào (xem mục Execution Plan).

8. Tránh N+1 query: thay vì chạy 1 query lấy danh sách rồi lặp mỗi dòng chạy thêm 1 query, hãy gộp bằng JOIN hoặc `WHERE id IN (...)`.
```sql
-- N+1: 1 query lấy user, rồi mỗi user 1 query lấy order
-- Gộp lại còn 1 query
...
9. Chọn kiểu dữ liệu gọn và đúng: dùng INT thay vì VARCHAR cho khóa, DATETIME thay vì chuỗi cho thời gian. Kiểu nhỏ hơn thì index nhỏ hơn, so sánh nhanh hơn.

10. Cập nhật thống kê bảng bằng `ANALYZE` (PostgreSQL) hoặc `ANALYZE TABLE` (MySQL) để optimizer chọn đúng kế hoạch; thống kê cũ dễ khiến database bỏ qua index.

Nguồn tham khảo:
- [Dremio – SQL Query Optimization](https://www.dremio.com/blog/sql-query-optimization/)
- [MySQL – Optimizing SELECT Statements](https://dev.mysql.com/doc/refman/8.0/en/select-optimization.html)
- [Use The Index, Luke! – How to Index](https://use-the-index-luke.com/)
...
- Mỗi node có cost=start..total (chi phí ước lượng), rows (số dòng ước lượng), width (kích thước dòng). Với EXPLAIN ANALYZE có thêm actual time và số dòng thật; lệch lớn giữa ước lượng và thực tế thường là do thống kê cũ.

Các kiểu JOIN trong plan:
- Nested Loop: quét bảng ngoài, mỗi dòng dò bảng trong. Nhanh khi một bên nhỏ và bên kia có index.
- Hash Join: dựng bảng băm từ một bên rồi dò bên kia. Tốt cho join hai bảng lớn không có index phù hợp.
- Merge Join: trộn hai bên đã sắp xếp theo khóa join. Tốt khi dữ liệu đã có sẵn thứ tự.
...
Nếu index chứa đủ mọi cột mà query cần (cả cột lọc lẫn cột trả về), database lấy dữ liệu ngay từ index mà không phải đọc bảng gốc.
```sql
-- Query chỉ cần user_id, status, total
CREATE INDEX idx_cover ON orders(user_id, status, total);
SELECT total FROM orders WHERE user_id = 1 AND status = 'paid';
-- EXPLAIN sẽ hiện "Using index" (MySQL), tức không cần đọc bảng
```
...
SAVEPOINT (điểm lưu tạm):
Cho phép rollback về một mốc giữa transaction thay vì hủy toàn bộ.
```sql
START TRANSACTION;
UPDATE accounts SET balance = balance - 100 WHERE id = 'A';
SAVEPOINT after_debit;
UPDATE accounts SET balance = balance + 100 WHERE id = 'B';
ROLLBACK TO after_debit;  -- hủy riêng bước cộng B, giữ bước trừ A
COMMIT;
```

Lưu ý autocommit: mặc định mỗi câu lệnh là một transaction tự commit ngay. Muốn gom nhiều lệnh thành một khối phải mở transaction rõ ràng bằng `START TRANSACTION` (hoặc `BEGIN`).
...
- Gap lock và next-key lock (InnoDB): khóa cả khoảng trống giữa các dòng chứ không chỉ dòng đang có, nhằm chặn việc chèn dòng mới vào khoảng đó. Đây là cách MySQL REPEATABLE READ chặn phantom read.
...
Nguồn tham khảo:
- [Use The Index, Luke! – We need tool support for keyset pagination (no-offset)](https://use-the-index-luke.com/no-offset)
...
-- Boolean mode: điều khiển bắt buộc/loại trừ từ khóa
SELECT * FROM posts
WHERE MATCH(title, content) AGAINST('+database -mysql' IN BOOLEAN MODE);
```
MySQL có ba chế độ: natural language (mặc định, xếp theo độ liên quan), boolean (dùng +, -, * để bắt buộc/loại trừ/tiền tố), và query expansion.
